...
def plot_channel_importance_ranking(df, save_path):
    """Plots a grouped bar chart ranking channels by their mean importance."""
    # Sort the dataframe by the total combined importance to rank them cleanly
    df['Total_Importance'] = df['Mean_Importance_T0'] + df['Mean_Importance_T1']
    df_sorted = df.sort_values(by='Total_Importance', ascending=False)

    fig, ax = plt.subplots(figsize=(14, 7))
    x = np.arange(len(df_sorted))
    width = 0.35

    ax.bar(x - width/2, df_sorted['Mean_Importance_T0'], width, label='Timepoint 0', color='#1f77b4')
    ax.bar(x + width/2, df_sorted['Mean_Importance_T1'], width, label='Timepoint 1', color='#ff7f0e')

    ax.set_ylabel('Mean Importance (Gradient Magnitude)')
    ax.set_title('Channel Importance Ranking (Sorted by Total Importance)')
    ax.set_xticks(x)
    ax.set_xticklabels(df_sorted['Channel'], rotation=45, ha='right')
    ax.legend()

    plt.tight_layout()
    plt.savefig(save_path, dpi=300)
    plt.close()
    print(f"Saved channel importance ranking plot to {save_path}")
...
if __name__ == "__main__":
    ...
    print(f"Model Prediction Probabilities: Class 0: {pred_prob[0]:.4f}, Class 1: {pred_prob[1]:.4f}")
     ...
     # 4. Extract Mean Channel Importance & Save to CSV
    mean_importance_0 = np.mean(cam_0, axis=1)
    mean_importance_1 = np.mean(cam_1, axis=1)
    
    df_importance = pd.DataFrame({
        'Channel': channel_names,
        'Mean_Importance_T0': mean_importance_0,
        'Mean_Importance_T1': mean_importance_1
    })
    
    csv_save_path = "channel_mean_importance.csv"
    df_importance.to_csv(csv_save_path, index=False)
    print(f"Saved mean channel importance to {csv_save_path}")
    ...
    # 6. Plot the Ranking
    plot_channel_importance_ranking(df_importance, save_path="channel_importance_ranking.png")